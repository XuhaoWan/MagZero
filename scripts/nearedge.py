...
class BesselBasisLayer(torch.nn.Module):
    ...
    def forward(self, dist: Tensor) -> Tensor:
        ...
        # dist = dist.clamp(min=1e-7, max=self.cutoff - 1e-7)
        # dist_normalized = dist.unsqueeze(-1) / self.cutoff
        # assert not torch.isnan(dist_normalized).any(), "归一化距离含NaN!"
        
        # envelope_output = self.envelope(dist_normalized)
        # assert not torch.isnan(envelope_output).any(), "Envelope输出含NaN!"
        
        # freq_dist = self.freq * dist_normalized
        # if torch.isnan(freq_dist).any():
        #     print("freq!!!!!!!", self.freq)
        # assert not torch.isnan(freq_dist).any(), "频率与距离乘积含NaN!"
        
        # sin_output = torch.sin(freq_dist)
        # assert not torch.isnan(sin_output).any(), "正弦输出含NaN!"
        
        # rbf = envelope_output * sin_output
        # return rbf
...
class SphericalBasisLayer(torch.nn.Module):
    ...
    def forward(self, dist: Tensor, angle: Tensor, idx_kj: Tensor) -> Tensor:
        ...
        # print("sbfdev:", dist.device, rbf.device, cbf.device)
        return out
...
class EmbeddingBlock(torch.nn.Module):
    def __init__(self, num_radial: int, hidden_channels: int, act: Callable):
        ...
        self.emb = Embedding(95, hidden_channels) #95 is all atoms number
...
class EmbeddingLayer(torch.nn.Module):
    def __init__(self, num_radial: int, hidden_dim: int, act: Callable, node_feat_dim: int):
        super().__init__()
        self.act = act

        self.proj = Linear(node_feat_dim, hidden_dim) 
        self.lin_rbf = Linear(num_radial, hidden_dim)
    ...
    def forward(self, x: Tensor, rbf: Tensor, i: Tensor, j: Tensor) -> Tensor:
        #assert not torch.isnan(x).any(), "输入x含NaN!"
        #assert not torch.isinf(x).any(), "输入x含Inf!"
        #assert not torch.isnan(rbf).any(), "输入rbf含NaN!"
        #assert not torch.isinf(rbf).any(), "输入rbf含Inf!"
        x = self.proj(x)
        # if torch.isnan(x).any():
        #     print("self.proj", self.proj.weight)
        rbf = self.act(self.lin_rbf(rbf))

        # if torch.isnan(rbf).any():
        #     print("!!!!!!!!!!!!!!!!!!!!!!!nan rbf")        
        return self.act(self.lin(torch.cat([x[i], x[j], rbf], dim=-1)))
